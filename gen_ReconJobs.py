...
recon_sim_job_nums = [i for i in range(int(np.ceil(len(sim_nums)*len(p.exps_config)*len(p.moments)*len(rdn0_sets))/nrecon_per_job))]
recon_rot_job_nums = [i for i in range(int(np.ceil(len(rot_nums)*len(p.exps_config)*len(p.moments)*len(rdn0_sets))/nrecon_per_job))]
...
for i, recon_job in enumerate(recon_sim_job_nums):
    ...
    if add_noise == True:
        for recon_param in recon_param_chuncks[i]:
            f.write('OMP_NUM_THREADS=%s python recon.py --cmb_map \'%s\' --experiment \'%s\' --nlev_t %s --beam_arcmin %s --ellmin %s --ellmax %s --delta_L %s --rdn0_set %s --add_noise & sleep 1\n' %(OMP_NUM_THREADS, recon_param['cmb_map'], recon_param['experiment'], recon_param['nlev_t'], recon_param['beam_arcmin'], recon_param['ellmin'], recon_param['ellmax'], recon_param['delta_L'], recon_param['rdn0_set']))
    else:
        for recon_param in recon_param_chuncks[i]:
            f.write('OMP_NUM_THREADS=%s python recon.py --cmb_map \'%s\' --experiment \'%s\' --nlev_t %s --beam_arcmin %s --ellmin %s --ellmax %s --delta_L %s --rdn0_set %s & sleep 1\n' %(OMP_NUM_THREADS, recon_param['cmb_map'], recon_param['experiment'], recon_param['nlev_t'], recon_param['beam_arcmin'], recon_param['ellmin'], recon_param['ellmax'], recon_param['delta_L'], recon_param['rdn0_set']))
...
for i, recon_job in enumerate(recon_rot_job_nums):
    ...
    f.write('#SBATCH --ntasks-per-node=%s\n' %n_tasks_per_node)
    f.write('#SBATCH --license=SCRATCH\n')
    f.write('#SBATCH --output=%sslurmjobRot.log.%d\n' %(job_path, recon_job))
    f.write('\n')
    f.write('cd %s\n\n' %src_path)
