...
plt.errorbar(ell_bin, bias_mean, yerr=bias_err, label=r"CMB S4")
plt.errorbar(ell_bin, bias_mean_s3, yerr=bias_err_s3, label=r"CMB S3")
...
fig = plt.figure(dpi=140)
ax = fig.add_subplot(111)
ax.errorbar(ell_bin, bias_n6_mean/10, yerr=bias_n6_err/10, fmt=".",label=r'$(A_{\mathrm{CB}}=10^{-6})/10$')
ax.errorbar(ell_bin+30, bias_n7_mean, yerr=bias_n7_err, fmt=".", label=r'$A_{\mathrm{CB}}=10^{-7}$')
ax.errorbar(ell_bin+60, bias_n8_mean*10, yerr=bias_n8_err*10, fmt=".", label=r'$(A_{\mathrm{CB}}=10^{-8})\times 10$')
ax.set_xlabel("$L$")
ax.set_ylabel('$(\Delta \hat{{C}}^{\mathrm{\phi \phi}}_{L})_{\mathrm{rot}}/C^{\mathrm{\phi \phi}}_{L}$')
ax.legend()
# plt.savefig("bias_scaling.pdf", bbox_inches='tight')
